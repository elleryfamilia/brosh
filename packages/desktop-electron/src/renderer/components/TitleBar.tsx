...
export function TitleBar({
  ...
  const moreMenuItems = useMemo(() => {
    ...
    if (isSingleTerminal && singleTab && onMcpToggle) {
      items.push({
        id: "mcp",
        label: singleTab.hasMcpSession ? "Disable MCP for this Terminal" : "Access this Terminal via MCP",
        icon: <McpIcon isActive={singleTab.hasMcpSession} size={14} />,
        indicator: "mcp",
        indicatorActive: singleTab.hasMcpSession,
        onClick: () => onMcpToggle(singleTab.sessionId),
      });
    }
    if (onOpenSettings) {
      items.push({
    ...
    return items;
  }, [isSingleTerminal, singleTab, onMcpToggle, onOpenSettings]);
          ...
          {showTabs ? (
            ...
            <div className={`title-bar-single ${isSingleTerminal ? 'title-bar-single-merged' : ''}`}>
              <div className="title-bar-single-title">
                ...
                {singleTab.hasMcpSession && <McpIcon isActive={true} size={12} />}
              </div>
              {isSingleTerminal ? (
