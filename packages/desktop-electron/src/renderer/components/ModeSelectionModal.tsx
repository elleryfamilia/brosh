...
export const ModeSelectionModal: React.FC<ModeSelectionModalProps> = ({
  ...
  const itemRefs = useRef<Map<string, HTMLDivElement | HTMLLabelElement | HTMLButtonElement | null>>(new Map());
  ...
  const currentNavIndex = navItems.findIndex(item =>
    ...
    (item.section === 'mode' ? item.index === (navTarget as { section: 'mode'; index: number }).index :
     item.section === 'network' ? item.index === (navTarget as { section: 'network'; index: number }).index :
     item.groupIndex === (navTarget as { section: 'permission'; groupIndex: number; pathIndex?: number }).groupIndex &&
     item.pathIndex === (navTarget as { section: 'permission'; groupIndex: number; pathIndex?: number }).pathIndex)
  );
  ...
  useEffect(() => {
    if (isOpen && dialogRef.current) {
      // Wait for browser to paint, then focus after a delay to ensure window is ready
  ...
  // Recapture focus when window regains focus (fixes modal becoming unresponsive after window switch)
  useEffect(() => {
    if (!isOpen) return;

    const handleWindowFocus = () => {
      // When window regains focus, ensure the modal dialog has focus
      setTimeout(() => {
  ...
  // Handle overlay click - refocus dialog if clicking outside content
  const handleOverlayClick = (e: React.MouseEvent) => {
    handleInteraction();
    // If clicking the overlay (not the dialog), refocus dialog
  ...
  return (
    <div className="mode-modal-overlay" onClick={handleOverlayClick}>
      {showLogo && (
      ...
      <div className="tui-mode-dialog" tabIndex={-1} ref={dialogRef}>
        ...
        <div className="tui-mode-footer">
          ...
          <button className="tui-btn" onClick={() => { handleInteraction(); handleContinue(); }}>Continue</button>
        </div>
      </div>
    </div>
  );
