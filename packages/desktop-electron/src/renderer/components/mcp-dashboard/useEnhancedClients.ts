...
export function useEnhancedClients(): UseEnhancedClientsReturn {
  ...
  // Pauses when window is hidden to save battery
  useEffect(() => {
    ...
    const startCleanup = () => {
      if (!interval) {
        interval = setInterval(doCleanup, 30000); // Every 30 seconds (was 10s)
      }
    };
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        // Run cleanup immediately on visibility restore, then resume interval
        doCleanup();
        startCleanup();
      }
    };
