...
import { isTerminalPane, isPendingPane, isSplitPane, isEditorPane } from "../types/pane";
...
export function getAllTerminalPanes(root: Pane): TerminalPane[] {
  ...
  if (isPendingPane(root) || isEditorPane(root)) {
    return [];
  }
...
function computePanePositions(
  ...
  if (isEditorPane(root)) {
    // Include editor panes in navigation
    return [{ paneId: root.id, x, y, width, height }];
  }
...
/**
 * Create a new editor pane for viewing/editing a file
 */
export function createEditorPane(
  filePath: string,
  isDiff = false,
  diffSource: DiffSource = "git-head"
): EditorPane {
  return {
    id: `editor-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
    type: "editor",
    filePath,
    isDiff,
    diffSource,
...
/**
 * Update the diff source of an editor pane
 */
export function updateEditorDiffSource(
  root: Pane,
  paneId: string,
  diffSource: DiffSource
): Pane {
  return transformPaneInTree(root, paneId, (pane) => {
    if (isEditorPane(pane)) {
      return { ...pane, diffSource };
    }
    return pane;
  });
}
...
/**
 * Find the first editor pane in the tree
 */
export function getFirstEditorPane(root: Pane): EditorPane | null {
  if (isEditorPane(root)) {
    return root;
  }
  if (isSplitPane(root)) {
    const inFirst = getFirstEditorPane(root.first);
    if (inFirst) return inFirst;
    return getFirstEditorPane(root.second);
  }
  return null;
}

/**
 * Update an editor pane's file path and diff settings
 */
export function updateEditorPaneFile(
  root: Pane,
  paneId: string,
  filePath: string,
  isDiff: boolean,
  ...
  return transformPaneInTree(root, paneId, (pane) => {
    if (isEditorPane(pane)) {
      return { ...pane, filePath, isDiff, diffSource };
    }
    return pane;
  });
}
...
export function splitPane(
  ...
  newPane: TerminalPane | PendingPane | EditorPane,
  ratio: number = 0.5
): Pane {
...
/**
 * Get the first focusable pane (terminal or editor) in the tree
 */
export function getFirstFocusablePane(root: Pane): Pane | null {
  if (isTerminalPane(root) || isEditorPane(root)) {
    return root;
  }
  ...
  const first = getFirstFocusablePane(root.first);
  if (first) return first;

  return getFirstFocusablePane(root.second);
}
