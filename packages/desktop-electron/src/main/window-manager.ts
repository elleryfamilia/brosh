...
export class WindowManager {
  ...
  async createWindow(): Promise<BrowserWindow> {
    ...
    // Initialize MCP server instance on first window (but don't start listening yet)
    // The server only starts when user explicitly enables MCP on a terminal
    if (!this.mcpServer) {
      this.mcpServer = new McpServer(this);
      // Don't auto-start - wait for user to enable MCP via mcp:start IPC
    }

    // Initialize and start IDE protocol server on first window
    if (!this.ideProtocolServer) {
      this.ideProtocolServer = new IdeProtocolServer(this);
      this.ideProtocolServer.start([app.getPath("home")]).catch((err) => {
        console.error("Failed to start IDE protocol server:", err);
      });
    }
    ...
    // Link terminal bridge to IDE protocol server (for diagnostics)
    bridge.setIdeProtocolServer(this.ideProtocolServer);
    ...
    window.on('blur', () => {
      this.broadcast('window:focus-changed', { focused: false });
      const managed = this.windows.get(windowId);
  ...
  /**
   * Get the IDE protocol server instance
   */
  getIdeProtocolServer(): IdeProtocolServer | null {
    return this.ideProtocolServer;
  }
  ...
  /**
   * Handle system suspend (sleep) - pause all background activities
   */
  handleSystemSuspend(): void {
    // Pause MCP server socket watcher
    this.mcpServer?.handleSystemSuspend();
    this.ideProtocolServer?.handleSystemSuspend();

    for (const managed of this.windows.values()) {
      managed.bridge.handleSystemSuspend();
    }
  }

  /**
   * Handle system resume (wake) - restart background activities
   */
  handleSystemResume(): void {
    // Resume MCP server socket watcher
    this.mcpServer?.handleSystemResume();
    this.ideProtocolServer?.handleSystemResume();

    for (const managed of this.windows.values()) {
      managed.bridge.handleSystemResume();
    }
  }
  ...
  /**
   * Dispose of the IDE protocol server
   */
  private disposeIdeProtocolServer(): void {
    if (this.ideProtocolServer) {
      this.ideProtocolServer.dispose();
      this.ideProtocolServer = null;
    }
  }
