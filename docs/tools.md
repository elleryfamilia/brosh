...
brosh exposes MCP tools that allow AI assistants like Claude Code to interact with terminal sessions. These tools work with both the desktop app's built-in MCP server and the standalone CLI MCP server.

This document provides complete API documentation for each tool.
...
- Default output directory: `~/.local/state/brosh/recordings/`
- Override with `BROSH_RECORD_DIR` environment variable
