...
This page covers CLI configuration options. The desktop app has a Settings panel (<kbd>Cmd</kbd>+<kbd>,</kbd>) for configuring themes, fonts, terminal dimensions, and other options visually.

brosh CLI can be configured via command-line arguments when starting the server.
...
# Custom dimensions
brosh --cols 80 --rows 24

# Specific shell
brosh --shell /bin/zsh

# Combined options
brosh --cols 100 --rows 30 --shell /usr/local/bin/fish
...
```bash
# Record with defaults (saves to ~/.local/state/brosh/recordings/)
brosh --record

# Record to specific directory
brosh --record --record-dir=./my-recordings

# Record only on failure
brosh --record=on-failure

# Custom idle time limit (5 seconds max between events)
brosh --record --idle-time-limit=5
```
...
Output:
```
brosh - A modern terminal with built-in AI integration via MCP
...
Example:
  brosh --cols 80 --rows 24 --shell /bin/zsh
```
...
```json
{
  "mcpServers": {
    "terminal": {
      "command": "node",
      "args": ["/path/to/brosh/dist/index.js"]
    }
  }
}
```
...
brosh uses this priority for shell selection:
...
| `TERM` | Set to `xterm-256color` by brosh |
...
2. `BROSH_RECORD_DIR` environment variable (if set)
3. `$XDG_STATE_HOME/brosh/recordings` (if XDG_STATE_HOME is set)
4. `~/.local/state/brosh/recordings` (fallback)

### Customizing Environment

Currently, environment variables are inherited from the process running brosh. To customize:
...
The terminal session starts in the current working directory of the brosh process.

```bash
# Start in a specific directory
cd /path/to/project && node /path/to/brosh/dist/index.js
```
...
brosh identifies itself as `xterm-256color`, which:
...
Currently, brosh supports a single terminal session. The session is created on first tool use and persists until shutdown.
