...
brosh supports recording terminal sessions to [asciicast v2](https://docs.asciinema.org/manual/asciicast/v2/) format, compatible with [asciinema](https://asciinema.org/) for playback and sharing.

> **Desktop App**: The GUI also supports recording via the status bar and terminal header controls.
...
Start brosh with recording enabled to capture the entire session.
...
1. `BROSH_RECORD_DIR` environment variable (if set)
2. `$XDG_STATE_HOME/brosh/recordings` (if XDG_STATE_HOME is set)
3. `~/.local/state/brosh/recordings` (fallback)
...
```json
{
  ...
  "path": "/Users/you/.local/state/brosh/recordings/terminal-1769449938208-63f64b587bd8817a.cast",
  "format": "v2",
  "mode": "always",
  "maxDuration": 3600,
...
5. stopRecording: {"recordingId": "abc123"}
   → Recording saved to ~/.local/state/brosh/recordings/
```
...
|----------|-------------|
| `BROSH_RECORD_DIR` | Override default recording output directory |
| `XDG_STATE_HOME` | XDG base directory for state files (used if BROSH_RECORD_DIR not set) |

### Example

```bash
# Set default recording directory in .bashrc/.zshrc
export BROSH_RECORD_DIR=~/my-recordings

# Or override for a single session
BROSH_RECORD_DIR=/tmp/debug-session brosh --record
```
...
# Play recording
asciinema play ~/.local/state/brosh/recordings/terminal-*.cast
...
brosh --record --max-duration=300 --inactivity-timeout=60

# Record with no inactivity limit (for long-running processes)
brosh --record --inactivity-timeout=0
```
...
4. **Set a default directory** - Configure `BROSH_RECORD_DIR` in your shell profile
